...
int gpio_psel_number   = GPIO_EXT_BUCK_EN_PIN;		//GPIO_CHR_PSEL_PIN; 
int gpio_psel_adapter_mode = GPIO_EXT_BUCK_EN_PIN_M_GPIO;	//GPIO_CHR_PSEL_PIN_M_GPIO;
int gpio_psel_usb_mode  = GPIO_EXT_BUCK_EN_PIN_M_GPIO;		//GPIO_CHR_PSEL_PIN_M_GPIO;
...
static void hw_bc11_dump_register(void)
{
	...
	kal_uint32 reg_num = MT6332_CHR_CON18;
	kal_uint32 i = 0;

	for(i=reg_num ; i<=MT6332_CHR_CON19 ; i+=2)
	{
...
static kal_uint32 charging_set_power_off(void *data)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "[bq24296]charging_set_power_off=%d\n");
    mt_power_off();
